...
module tb_astral;
  ...
  initial begin
    ...
    if (car_phy_sel_valid) begin
      ...
      fix.select_phy_startup(car_phy_sel);
    end
    ...
    if (chs_preload_elf != "" || chs_boot_hex != "") begin
      ...
      // Understand if we are using a DRAM-mapped binary
      // to configure the HyperBus
  ...
  if (CarfieldIslandsCfg.safed.enable) begin : gen_safed_tb
    ...
    initial begin
      ...
      if (safed_preload_elf != "") begin
        ...
        if (fix.DutCfg.SerialLink) begin
          $display("[TB] INFO: Configuring Hyperbus through serial link.");
